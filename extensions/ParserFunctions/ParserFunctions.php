...
class ExtParserFunctions {
	...
	/* tav : patched this function with a max variable */
	function ifexistCommon( &$parser, $frame, $title = '', $then = '', $else = '', $max = 0 ) {
	...
	function ifexistObj( &$parser, $frame, $args ) {
		...
		/* tav */
		$max = isset($args[3]) ? trim($frame->expand($args[3])) : 0;

		$result = $this->ifexistCommon( $parser, $frame, $title, $then, $else, $max );
		if ( $result === null ) {
			return '';
		} else {
			return trim( $frame->expand( $result ) );
